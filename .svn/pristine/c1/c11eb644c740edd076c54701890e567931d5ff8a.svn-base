...
public class HW_GMSS_CLDPREANA_CLDNCD_INDEX extends VariantValueInvoke {
    ...
    private void initMap(String dbFile, String sNename1) {
        DataQueryUtils utils = DataQueryUtils.getInstance();
        String sql = "select '" + sNename1 + "' SERVER,BUSI_CITY,MGWINDEX from HW_GMSC_CITY_RELATION where SERVER = '" + sNename1 + "' ";
        List<Map<String, ?>> resultList = utils.getLocalData(sql, dbFile);
        for (Map temp : resultList) {
