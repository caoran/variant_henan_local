...
public class SCPINGT extends VariantValueInvoke {
    ...
    private void initMap(String dbFile) {
        ...
        String sql = "SELECT DEVICENAME,GROUP_CONCAT(SCPINGT) SCPINGT  FROM HW_UAP_SCPMAP WHERE cast(SCPINGT as int) BETWEEN 101 AND 65535 GROUP BY DEVICENAME";
        List<Map<String, ?>> resultList = utils.getLocalData(sql, dbFile);
        for (Map<String, ?> temp : resultList) {
            String deviceName = temp.get("DEVICENAME").toString();
            String scpno = temp.get("SCPINGT").toString().trim();
