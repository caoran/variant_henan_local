 ...
 * note:
 */
package com.boco.soap.variant.henan.local.scp.casmsisdntoid.hw;
...
public class startGtBegin extends VariantValueInvoke {
    ...
    public String[] getValues(INeElement arg0, IInstructionParameter arg1, Map<String, ?> data, String arg3) {
        ...
        if (null == o) {
            ...
            List<String> resultList = this.map.get(data.get("PROVINCE").toString());
    ...
    private void initMap(String dbFile, String deviceName) {
        ...
        String sql = "SELECT PROVINCE,START_GT FROM  TCM_CMCC_BOSS_GT m WHERE START_GT not IN (SELECT MSISDNSTART FROM  HW_SCP_CAS_MSISDNTOID q WHERE DEVICENAME='"
                + deviceName + "') ORDER BY START_GT ";
        List<Map<String, ?>> resultList = utils.getLocalData(sql, dbFile);

        for (Map<String, ?> temp : resultList) {
            List<String> tempList = this.map.get(temp.get("PROVINCE").toString());
            if (tempList == null) {
                tempList = new ArrayList<String>();
                this.map.put(temp.get("PROVINCE").toString(), tempList);
            }
