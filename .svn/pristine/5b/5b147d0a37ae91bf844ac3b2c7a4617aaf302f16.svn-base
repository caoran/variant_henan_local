 ...
 * 
 */
public class HW_GMSS_CNACLD_RSNAME_ZZIMS3_B extends VariantValueInvoke {
	...
	public String[] getValues(INeElement ne, IInstructionParameter arg1,
		...
		if (null == oCity) {
			...
			String accessArea = oAccessArea != null ? oAccessArea.toString()
					: "";
			String backName = deviceMap.get(ne.getName());
			String backCovercitys = map.get(backName);
			// 如果覆盖地市包含接入地
			if (!accessArea.equals("") && (accessArea.contains(oCity.toString())||oCity.toString().equals("全省")) && backCovercitys.contains(accessArea)) {
				if (!routeName.equals("")) {
					route = routeName;
				} else {
					route = "ZZIMS3-" + route;
