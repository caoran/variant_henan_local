...
public class REFERINDEX_BER extends VariantValueInvoke {
	...
	public String[] getValues(INeElement arg0, IInstructionParameter arg1, Map<String, ?> data, String dbFile) {
		...
		if (null == o) {
			...
			while (iter.hasNext()) {
				...
				// 判断现网是否已存在该APN参考号
				if (entry.getKey().startsWith("BER") && entry.getKey().contains(_dnString)) {
	...
	private void getReferIndex(String dbFile, String deviceName) {
		...
		// 参考号：ER-BNKPCRF:321-330;
		sql = "SELECT MAX(CAST(REFERINDEX AS INT)) MAX_REFERINDEX  FROM HW_LDRA_RTAPN  WHERE DEVICENAME='" + deviceName + "' AND CAST(REFERINDEX AS INT) >= 321  AND CAST(REFERINDEX AS INT) <= 330";
		...
		// 参考号：ER-BERPCRF:331+;
		sql = "SELECT MAX(CAST(REFERINDEX AS INT)) MAX_REFERINDEX  FROM HW_LDRA_RTAPN  WHERE DEVICENAME='" + deviceName + "' AND CAST(REFERINDEX AS INT) >= 331";
		resultList = utils.getLocalData(sql, dbFile);
