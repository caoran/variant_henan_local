...
public class startGtBegin extends VariantValueInvoke {
    ...
    private void initMap(String dbFile, String deviceName) {
        ...
        String sql = "SELECT BUSI_CITY,START_GT FROM  TCM_CMCC_BOSS_GT m WHERE START_GT not IN (SELECT STARTNO FROM  HW_SCP_MVPN q WHERE DEVICENAME='" + deviceName + "') AND BUSI_CITY IS NOT NULL  ORDER BY START_GT ";
