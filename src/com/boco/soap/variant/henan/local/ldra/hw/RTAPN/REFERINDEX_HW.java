...
public class REFERINDEX_HW extends VariantValueInvoke {
	...
	public String[] getValues(INeElement arg0, IInstructionParameter arg1, Map<String, ?> data, String dbFile) {
		...
		if (null == o) {
			...
			while (iter.hasNext()) {
				...
				// 判断现网是否已存在该APN参考号
				if (entry.getKey().startsWith("HW") && entry.getKey().contains(_dnString)) {
	...
	private void getReferIndex(String dbFile, String deviceName) {
		...
		// SQL语句必须满足sqlite的语法规范
		// 参考号：HW-HWPCRF:1-20;
		String sql = "SELECT MAX(CAST(REFERINDEX AS INT)) MAX_REFERINDEX  FROM HW_LDRA_RTAPN  WHERE DEVICENAME='" + deviceName + "' AND CAST(REFERINDEX AS INT) >=1  AND CAST(REFERINDEX AS INT) <= 20";
		...
		// 参考号：HW-BNKPCRF:21-30；
		sql = "SELECT MAX(CAST(REFERINDEX AS INT)) MAX_REFERINDEX  FROM HW_LDRA_RTAPN  WHERE DEVICENAME='" + deviceName + "' AND CAST(REFERINDEX AS INT) >= 21  AND CAST(REFERINDEX AS INT) <= 30";
		...
		// 参考号： HW-BERPCRF:31-100;
		sql = "SELECT MAX(CAST(REFERINDEX AS INT)) MAX_REFERINDEX  FROM HW_LDRA_RTAPN  WHERE DEVICENAME='" + deviceName + "'  AND CAST(REFERINDEX AS INT) >= 31  AND CAST(REFERINDEX AS INT) <= 100";
		resultList = utils.getLocalData(sql, dbFile);
