...
public class LDRA_RTAPN_REFERINDEX_BER extends VariantValueInvoke {
	...
	private void initiCityPcrfMap(String dbFile) {
		...
		String sqlString = "SELECT BUSI_CITY,APN_NEXTINDEX_BER FROM TCM_HN_PCRF_AREA ";

		List<Map<String, ?>> resultList = utils.getLocalData(sqlString, dbFile);
		for (Map<String, ?> itemMap : resultList) {
			_cityPcrfMap.put(itemMap.get("BUSI_CITY").toString().trim(), itemMap.get("APN_NEXTINDEX_BER").toString().trim());
		}
	}
