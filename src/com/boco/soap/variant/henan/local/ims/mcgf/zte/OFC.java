...
public class OFC extends VariantValueInvoke {
	...
	public String[] getValues(INeElement arg0, IInstructionParameter arg1,
		...
		if (objNeType.toString().trim().toUpperCase().equals("MGCF")
			&& objActTbName.toString().trim().toUpperCase().equals("SIPOFC")
			&& paraName!=null) {
	...
	private void initMap(String dbFile) {
		...
		String sql = "SELECT DEVICENAME,GROUP_CONCAT(OFC) OFC FROM ZTE_MGCF_SIPOFC GROUP BY DEVICENAME";
		List<Map<String, ?>> resultList = utils.getLocalData(sql, dbFile);

		for (Map<String, ?> temp : resultList) {
			String deviceName=temp.get("DEVICENAME").toString();
			String identifiers=temp.get("OFC").toString();
